...
class DrawingPanel extends JPanel implements ComponentListener {
  ...
  DrawingPanel(NetworkPanel parent) {
	...
	public void drawNodes() {
		...
		while (nodes.hasMoreElements())
		{
			...
			if (state == State.PASSIVE || state == State.FOLLOWER) g.setColor(Node.ID_COLOUR_ON_BLACK);
			denominator = 3;
  ...
  protected void resizeBackgroundImage() {
	...
	protected void drawLinksAndMessages() {
		...
		Link link; NetViewerMessage msg;
		...
		while (links.hasMoreElements())
		{
			...
			if (link instanceof TorusLink)
				...
				//Draw link cost
				if(link.getCost()!=-1)
					g.drawString(Integer.toString(link.getCost()), link.getStartCoords().x + (link.getEndCoords().x-link.getStartCoords().x)/2,
						link.getStartCoords().y + (link.getEndCoords().y-link.getStartCoords().y)/2);
				//end
