...
class RingNodeFarAsCan extends Node {
	...
	private void asleep(String msg, int dir) {
	  initialize();
		...
		if (nodeId < msgInt) {
			...
			NetViewer.out.println("Node "+nodeId+" has been woken up, remained candidate, defeated "+msg+".");
		}
		else /* msg < id */ {
			become(general.State.PASSIVE);
			send(msg, Math.abs(dir-1)); // send in opposite direction
			NetViewer.out.println("Node "+nodeId+" has been woken up, become passive, and sent msg "+msg+" to the "+((Math.abs(dir-1)==0)?("RIGHT"):("LEFT"))+".");
		}
	}
	...
	private void candidate(String msg, int dir) {
		...
		if (nodeId < msgInt) {
			// do nothing; remain candidate
			NetViewer.out.println("Node "+nodeId+" remains candidate, defeats "+msg);
		}
		else if (msgInt < nodeId) {
			become(general.State.PASSIVE);
			send(msg, Math.abs(dir-1)); // send in opposite direction
			NetViewer.out.println("Node "+nodeId+" defeated. Became passive and forwarded "+msg+" to the "+((Math.abs(dir-1)==0)?("RIGHT"):("LEFT"))+".");
		}
		else { /* received id */
	...
	private void passive(String msg, int dir) {
		...
		send(msg, Math.abs(dir-1)); // send in opposite direction
		NetViewer.out.println("Passive node "+nodeId+" forwarded message "+msg+" to the "+((Math.abs(dir-1)==0)?("RIGHT"):("LEFT"))+".");
	}
