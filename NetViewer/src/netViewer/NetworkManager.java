...
class NetworkManager implements ActionListener {
	...
	public void addRandomDifferentLinksCost(){
		...
		while (links.hasMoreElements()){
			...
			i++;
			NetViewer.out.println("Il link dal nodo " + l.getNode(0) + " al nodo "+  l.getNode(1) + " ha costo: " + l.getCost() + ".");
		}
	}
...
//	}
  ...
  public void waitForThreadsToFinish() throws InterruptedException {
		...
		Link link; NetViewerMessage m; Enumeration messages;
	...
	private Node newNode(Integer id_) {
		...
		try {
			return (Node)Class.forName(this.getClass().getPackage().getName() + "." + nodeType).getDeclaredConstructor(integerClass).newInstance(id);
		} catch (Exception e) {
			e.printStackTrace();
			return null;
		}
  ...
  public void actionPerformed(ActionEvent e) {
		...
		while (messages.hasMoreElements())
		{
			...
			if (m.getNumNodesSeenByMessage() == -1)
				...
				link.send(m.getPacket().printString(), m.getDirection(), m.getNumNodesSeenByMessage());
		...
		while (links.hasMoreElements())
		{
			...
			for (int i=0; i<2; i++) { // once for right, once for left
				...
				if (!stringMsg.equals(""))
				{
					m = new NetViewerMessage(new StringMessage(stringMsg), i, link, true); // create a single message containing the text of all messages travelling in this direction on this link
