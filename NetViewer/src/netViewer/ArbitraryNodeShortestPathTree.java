...
public class ArbitraryNodeShortestPathTree extends Node {
	...
	ArbitraryNodeShortestPathTree(Integer ID) {
	...
	protected void initialize() {
		if (source) {
			...
			ackcount = links.size();
			for (int i = 0; i < links.size(); i++)
				// Notifico ai nodi del grafo che sono io la radice
	...
	/**
	 * Operazioni eseguite dal nodo che si trova nello stato INITIATOR al
	 * momento della ricezione di un msg
	 */
	private void init(String msg, Link link) {
		...
		if (msgStr.equals("Ack")) {
			ackcount--;
			NetViewer.out.println("Source has received an ack from node "
					+ link.getOtherNode(this).getNodeId());
			if (ackcount == 0) {
				iteration = 1;

				// calcola il miglior link usando la funzione v(x,y) nel ns.
				// caso y e' il link (x,y) e x e' sempre "this"
	...
	private int v(ArbitraryNodeShortestPathTree x, Link link) {
		return x.myDistance + link.getCost();
	}

	/**
	 * Operazioni eseguite da un nodo che si trova nello stato IDLE al momento
	 * della ricezione di un msg
	 */
	private void idle(String msg, Link link) {
		String[] msgArr = msg.split(" ");
		String msgStr = msgArr[0];

		// ogni metodo relativo ad uno stato inizialmente fa questa print per
		// visualizzare le azioni nei log
		NetViewer.out.println("Node " + getNodeId()
				+ " in the state IDLE has received msg: " + msgStr
				+ " from node: " + link.getOtherNode(this).getNodeId());
		...
		if (msgStr.equals("Notify")) {
			...
			send("Ack", link);
			NetViewer.out.println("Node " + getNodeId() + " sent an ACK msg");
			become(general.State.AWAKE);
		}
	}

	/**
	 * Operazioni eseguite da un nodo che si trova nello stato AWAKE al momento
	 * della ricezione di un msg
	 */
	private void awake(String msg, Link link) {
		...
		NetViewer.out.println("Node " + getNodeId()
				+ " in the state AWAKE has received msg: " + msgStr
				+ " from node: " + link.getOtherNode(this).getNodeId());
		...
		if (msgStr.equals("Expand")) {
			...
			if (links.size() > 1) {
				for (int i = 0; i < links.size(); i++) {
					if (!links.get(i).equals(parentLink)) // Invio il msg di
															// notifica a tutti
															// i miei vicini
															// eccetto il sender
						send("Notify", (Link) links.get(i));
				...
				NetViewer.out.println("Node " + getNodeId()
						+ " sent an ITERATION COMPLETED msg");
				become(general.State.ACTIVE);
			}
	...
	/**
	 * Operazioni eseguite da un nodo che si trova nello stato ACTIVE al momento
	 * della ricezione di un msg
	 */
	private void waiting_for_ack(String msg, Link link) {
		...
		NetViewer.out.println("Node " + getNodeId()
				+ " in the state WAITING_FOR_ACK has received msg: " + msgStr
				+ " from node: " + link.getOtherNode(this).getNodeId());

		if (msgStr.equals("Ack")) { // se riceve un ACK
			ackcount = ackcount - 1;
			if (ackcount == 0) { // se ho ricevuto tutti gli ack che dovevo
									// ricevere allora lo dico al mio padre e
									// divento ACTIVE
				send("IterationCompleted", parentLink);
				NetViewer.out.println("Node " + getNodeId()
						+ " sent an ITERATION COMPLETED msg");
	...
	private void active(String msg, Link link) {
		...
		NetViewer.out.println("Node " + getNodeId()
				+ " in the state ACTIVE has received msg: " + msgStr
				+ " from node: " + link.getOtherNode(this).getNodeId());

		if (msgStr.equals("IterationCompleted")) {
			if (!source) {
				send("IterationCompleted", parentLink);
				NetViewer.out.println("Source " + getNodeId()
						+ " sent an ITERATION COMPLETED msg");
			} else {
				iteration++;
				...
				NetViewer.out.println("Node " + getNodeId()
						+ " sent a START ITERATION msg");
				ComputeLocalMinimum();// Invoco la procedura Compute Local
										// Minimum
		...
		if (msgStr.equals("Expand")) {
			...
			if (exit.equals(mychoice) && !childLink.contains(mychoice)) {
				NetViewer.out.println("*****The " + mychoice
						+ " is in the shortest path tree *****");
				mychoice.setColor(Color.blue);
				childLink.add(mychoice); // Children = Children U {mychoice}
				unvisited.remove(mychoice); // Unvisited = Unvisited -
											// {mychoice}
			}
		}
	...
	private void computing(String msg, Link link) {
		...
		NetViewer.out.println("Node " + getNodeId()
				+ " in the state COMPUTING has received msg: " + msgStr
				+ " from node: " + link.getOtherNode(this).getNodeId());
		...
		if (msgStr.equals("MinValue")) {
			...
			NetViewer.out.println("Node " + getNodeId()
					+ " in the state COMPUTING childcount= " + childcount
					+ "  |Children|=" + childLink.size());
			NetViewer.out.println("Node " + getNodeId()
					+ " in the state COMPUTING");
	...
	private void checkForTermination() {
		if (minpath == 10000) {
			...
			// ********THE TRICK*******//
			if (exit.equals(mychoice) && !childLink.contains(mychoice)) {
				NetViewer.out.println("i am the node " + getNodeId()
						+ " *****The " + mychoice
						+ " is in the shortest path tree *****");
				mychoice.setColor(Color.blue);
				childLink.add(mychoice); // Children = Children U {mychoice}
			...
			NetViewer.out
					.println("Node " + getNodeId() + " sent an EXPAND msg");
			become(general.State.ACTIVE);
		}
	...
	private void ComputeLocalMinimum() {
		...
		if (source) {
			NetViewer.out.println("I AM THE SOURCE,id=" + getNodeId()
		...
		if (unvisited.size() == 0) { // if Unvisited è vuoto
			...
			int link_length = 0;
			Link u = unvisited.get(0); // link_length = v(x,y) = MIN{v(x,z):z
										// appartiene ad Unvisited}
			int minU = v(this, u);
