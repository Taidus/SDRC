...
class RingNodeAllTheWay extends Node {
	...
	/*
	 * Send a message; put it on the link in direction dir. Doesn't use the
	 * send() in Node because we need a 3rd parameter.
	 */
	protected void send(String msg, int dir, int numNodesSeenByMessage) {
		
		Message m = new AllTheWayMessage(msg, numNodesSeenByMessage);
		send(m,dir);
	}
	...
	public synchronized void receive(Message m, int dir) {
		...
		AllTheWayMessage am = (AllTheWayMessage)m;
		String msg = am.getMsg();
		int numNodesSeenByMessage = am.getNumNodesSeenByMessage();
		
		NetViewer.out.println("Node " + nodeId + " received message " + msg
				+ " on the " + ((dir == 0) ? ("RIGHT") : ("LEFT")) + ".");
		switch (state) {
		case general.State.ASLEEP:
			asleep(msg, dir, numNodesSeenByMessage);
			break;
		case general.State.AWAKE:
			awake(msg, dir, numNodesSeenByMessage);
			break;
		}
	}
	...
	private void asleep(String msg, int dir, int numNodesSeenByMessage) {
		initialize();
		send(msg, Math.abs(dir - 1), numNodesSeenByMessage + 1); // blindly
																	// forward
																	// ID
		...
		NetViewer.out.println("Node " + nodeId
				+ " has been woken up, forwarded " + msg + " to the "
				+ ((Math.abs(dir - 1) == 0) ? ("RIGHT") : ("LEFT")) + ". min: "
				+ minimum);
	}

	/*
	 * Process message received while state = AWAKE. dir is the direction from
	 * which the message arrived.
	 */
	private void awake(String msg, int dir, int numNodesSeenByMessage) {
		...
		if (msgInt != nodeId) {
			...
			if (ringsizeKnown)
				checkIfDone();
			NetViewer.out.println("Node " + nodeId + " forwarded " + msg
					+ " to the "
	...
	private class AllTheWayMessage implements general.Message {

		String msg;
		int numNodesSeenByMessage;

		public AllTheWayMessage(String msg, int numNodesSeenByMessage) {
			super();
			this.msg = msg;
