...
class TorusPanel extends DrawingPanel implements ComponentListener {
	...
	/*
	 * Draw a link. May have to draw two parts if it's a wrap-around link.
	 */
	protected void draw(TorusLink link) {
		g.setColor(Color.black);
		if (link.isWrapAround()) {
			g.drawLine(link.getStartCoordsA().x, link.getStartCoordsA().y,
					link.getEndCoordsA().x, link.getEndCoordsA().y); // 1st part
																		// of
																		// link
			g.drawLine(link.getStartCoordsB().x, link.getStartCoordsB().y,
					link.getEndCoordsB().x, link.getEndCoordsB().y); // 2nd part
																		// of
																		// link
		} else {
			g.drawLine(link.getStartCoords().x, link.getStartCoords().y,
					link.getEndCoords().x, link.getEndCoords().y);
		}
	}

	/*
	 * Draw a message on a wrap-around link. Figure out which half of the link
	 * it is on and how far along.
	 */
	protected void drawMsgOnWrapAroundLink(TorusLink link, NetViewerMessage msg) {
		...
		if (link.getStartCoordsA().x == link.getEndCoordsA().x) // up/down link
		{
			if (msg.getDirection() == Link.RIGHT) { // down
				if (percentDone <= 0.5) // msg is on the 1st part of the wrap
										// around link going down
					msg_y = link.getStartCoordsA().y + 2 * percentDone
							* link.lengthWrapAround();
				else
					// msg is on the 2nd part of the wrap around link going down
					msg_y = link.getStartCoordsB().y + 2 * (percentDone - 0.5)
							* link.lengthWrapAround();
				rotator = 3 * Math.PI / 4; // 135 degrees
			} else { // up
				if (percentDone <= 0.5) // msg is on the 2nd part of the wrap
										// around link going up
					msg_y = link.getEndCoordsB().y - 2 * percentDone
							* link.lengthWrapAround();
				else
					// msg is on the 1st part of the wrap around link going up
					msg_y = link.getEndCoordsA().y - 2 * (percentDone - 0.5)
							* link.lengthWrapAround();
			...
			((Graphics2D) g).rotate(Math.PI / 2 + rotator, msg_x, msg_y); // theta
																			// gets
																			// g
																			// in
																			// line
			...
			((Graphics2D) g).rotate(-(rotator + Math.PI / 2), msg_x, msg_y); // back
																				// to
																				// original
																				// position
			...
			g.drawString(msg.getPacket().printString(), (int) msg_x + 10,
					(int) msg_y + 10);
		} else // left/right link
		{
			if (msg.getDirection() == Link.RIGHT) {
				if (percentDone <= 0.5) // msg is on the 1st part of the wrap
										// around link going right
					msg_x = link.getStartCoordsA().x + 2 * percentDone
							* link.lengthWrapAround();
				else
					// msg is on the 2nd part of the wrap around link going
					// right
					msg_x = link.getStartCoordsB().x + 2 * (percentDone - 0.5)
							* link.lengthWrapAround();
				rotator = 3 * Math.PI / 4; // 135 degrees
			} else { // left
				if (percentDone <= 0.5) // msg is on the 2nd part of the wrap
										// around link going left
					msg_x = link.getEndCoordsB().x - 2 * percentDone
							* link.lengthWrapAround();
				else
					// msg is on the 1st part of the wrap around link going left
					msg_x = link.getEndCoordsA().x - 2 * (percentDone - 0.5)
							* link.lengthWrapAround();
				rotator = -Math.PI / 4; // 45 degrees
			}
			...
			// draw the message as an arrow
			g.setColor(NetViewerMessage.MESSAGE_COLOUR);
			((Graphics2D) g).rotate(0 + rotator, msg_x, msg_y); // theta gets g
																// in line with
																// the link, and
																// the rotator
																// places g at
			...
			g.fillRect((int) msg_x, (int) msg_y, 8, 2);
			g.fillRect((int) msg_x, (int) msg_y, 2, 8);
			((Graphics2D) g).rotate(-(rotator + 0), msg_x, msg_y); // back to
																	// original
																	// position
